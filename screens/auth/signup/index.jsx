...
export const SignUp = () => {
  const { signInWithGoogle: signInWithGoogleContext } = useSession();
  const { signInWithGoogle, isLoading: isLoadingGoogle } = useGoogleAuth();
  ...
  const [isLoadingGoogleSignup, setIsLoadingGoogleSignup] = useState(false);

  const handleGoogleSignup = async () => {
    try {
      setIsLoadingGoogleSignup(true);
      const googleUserData = await signInWithGoogle();
      await signInWithGoogleContext(googleUserData);
      router.replace("home");
    } catch (error) {
      console.error("Erro no cadastro com Google:", error);
      alert(error.message || "Erro ao fazer cadastro com Google");
    } finally {
      setIsLoadingGoogleSignup(false);
    }
  };
          ...
          <FormControl className="bg-gray-50 p-5 border rounded-lg border-outline-300 w-[95%]">
            <VStack space="xl">
              ...
              <Button 
                ...
                isDisabled={isLoadingGoogleSignup || isLoadingGoogle}
              >
                <VStack className="items-center gap-2">
                  
                  <ButtonText>
                    {isLoadingGoogleSignup || isLoadingGoogle ? "Carregando..." : "Cadastrar com Google"}
                  </ButtonText>
                </VStack>
              </Button>
            </VStack>
